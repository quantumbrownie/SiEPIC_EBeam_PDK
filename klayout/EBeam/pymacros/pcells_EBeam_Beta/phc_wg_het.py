# 2024-10-12 Justin Brown, adapted from previous phc_wg_w1 macro
# Photonic crystal waveguide allowing a different lattice constant in the x direction, for photonic crystal heterostructures
...
class phc_wg_het(pya.PCellDeclarationHelper):
    """
    Input: length, width
    """
    ...
    def __init__(self):
        ...
        super(phc_wg_het, self).__init__()

        self.param("a", self.TypeDouble, "lattice constant (microns)", default=0.744)
        self.param("ax", self.TypeDouble, "lattice constant for x (microns)", default=0.744)
        self.param(
            "n", self.TypeInt, "Number of holes in x direction", default= 2
        )
        ...
        self.param("etch_condition", self.TypeInt, "Etch = 1, No Etch = 2", default=2)
    ...
    def produce_impl(self):
        ...
        if wg_dis % 2 == 0:
            length_slab_x = (2 * n - 1) * ax
        else:
            length_slab_x = 2 * n * ax
